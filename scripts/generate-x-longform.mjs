...
const MACRO_TOPICS = [
  ...
  },
  {
    ...
    scene: 'SNSで同世代がお金の話を当たり前にしているのを見て焦る',
    cta_slug: 'fx-shoshinsha-guide',
    cta_label: 'FXを始める前に知っておきたいこと',
  },
  {
  ...
  {
    id: 'first-step-lost',
    emotion: 'お金を増やす方法を知りたいけど何から手を付ければいいかわからない',
    scene: 'お金の情報が多すぎてブックマークだけが増えていく状態',
    cta_slug: 'fx-shoshinsha-guide',
...
const TEMPLATES = {
  L1: {
    name: '失敗回避型（最拡散）',
    structure: `
1行目: 「FXで最初にやらかしやすい○つ」「○万円が○日で消える計算を出してみた」のように数字と失敗を入れる
2〜4行: 結論（何を避ければよかったか）
中盤: 失敗のメカニズム（ロスカット・レバレッジ・追証）を具体的な計算で見せる
注意点: 少額で始める理由
最後: ブログへのCTA`,
    firstLineHint: '数字と失敗のメカニズムを入れる（一人称の損失体験は書かない）。例: 「レバレッジ25倍だと、証拠金の何%の逆行でロスカットされるか計算してみた。」',
  },
  L2: {
    name: '比較・判断軸型（最保存）',
    structure: `
1行目: 「FX口座○社の公式条件を並べてわかった選び方」「迷う○口座を条件表で比較した」
中盤: スプレッド・スワップ・最低証拠金・MetaTrader対応状況を表形式か箇条書きで整理（「MT4対応」と丸めず、取引可否・EA可否まで条件付きで書く）
判断軸: どんな人が何を選ぶべきか（3パターン）
注意: 1社を強く推さず「向き不向き」で整理
最後: ブログへのCTA`,
    firstLineHint: '比較と保存欲求を誘う（「実際に使った」とは書かない）。例: 「国内FX4社の公式スプレッドを並べたら、選ぶ順番が変わった。」',
  },
  L3: {
...
よくある間違い: 1〜2個
最後: ブログへのCTA`,
    firstLineHint: 'ステップ感を出す（一人称の実行体験は書かない）。例: 「FXの確定申告、国税庁の資料をたどって5ステップに整理しました。」',
  },
  L4: {
    name: '調査・比較記録型（信頼構築）',
    structure: `
1行目: 「FXの○○を○社分調べ直した記録」「FXが怖いと言われる理由を、条件表から分解した」
背景: なぜ調べたか（読者からよく出る疑問・編集部自身が分からなかった点）
調べてわかったこと: 一次情報（各社公式・金融庁・国税庁）で確認できた事実だけを書く
落とし穴: 条件表の中で見落とされやすい項目を1〜2個
まだ分かっていないこと: 確認できていない点を正直に書く（透明性）
最後: フォローかブログへ
※ 一人称の売買体験・入金額・損益は書かない。書いてよいのは「調べた・比較した・確認した」だけ`,
    firstLineHint: '調査の記録として始める（自分の運用体験にしない）。例: 「国内FXの最低取引単位、5社分を公式ページで確認し直しました。」',
  },
  L5: {
...
2行: 選択肢を並べる
中盤: 判断軸を整理（なぜそちらか）
編集部の答え: tsumiba編集部（build in public・架空の体験談禁止）が調べた範囲でどちらを支持するかを、根拠つきで明かす
読者へのボール: 「あなたはどちらを選びましたか？」
最後: 参考になった人はリポスト or フォロー`,
    firstLineHint: '問いかけで始める（読者の職業・属性を決めつけない）。例: 「FXを1,000通貨で始めるか、1万通貨で始めるか。条件を並べて考えてみた。」',
  },
};
...
const longformResponse = await client.messages.create({
  ...
  max_tokens: 6000,
  system: `あなたはtsumiba編集部として、X Premium向けの長文投稿を書くライターです。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【編集部とは誰か】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
tsumiba（FX口座比較メディア）の編集部。各社公式サイト・金融庁・国税庁等の一次情報を突合し、スプレッド・スワップ・キャンペーン条件を比較検証して記事にしている。
架空の経歴・人生ストーリー・運用実績は語らない（架空個人ペルソナは2026-05-31に全廃）。
編集部は口座を実際に運用してはいない。「実際に使った」「開設して試した」「取引した」と書くのは虚偽なので禁止。
語ってよいのは、一次情報の確認・条件の突合・比較検証の結果だけ。
→ 編集部は「成功者」ではない。「読者の代わりに比較・検証する調査者」として書く。
...
「編集部がヒーローになってはいけない。読者がヒーローで、編集部はガイドだ」
→ 編集部の検証記事は「私がすごい」を伝えるためではなく、「あなたも同じ道を通れる」を示すために使う。
→ 読者の悩み・感情・状況を先に書いてから、編集部の経験を引き合いに出す。
...
■ エンパシーマップ（読者の内面を先に描く）：
See（読者が見ているもの）：SNSの資産形成投稿・インフレニュース・同世代がお金を動かしている話
Hear（読者が聞くこと）：「老後2,000万円問題」「物価上昇」「貯金だけでは増えない」
...
A（Affinity）：内的感情を代弁・共感。「それは当然です」「そこで迷う人は多いです」。煽らない・責めない・自分の体験に置き換えない
S（Solution）：解決策を具体的に。一次情報で確認できた数字・手順を入れる
...
幕3（試練と学習）：検証・調査で判明した落とし穴や意外な事実。具体的な数字・条件を入れる（架空の体験談は禁止）。
幕4（変容）：調べた結果、読者が何を判断できるようになるか。編集部の運用成果としては書かない。断定的な成功訴求はしない。
...
■ 透明性原則（信頼の最大の武器）：
調べても分からなかったこと・条件の不確かさ・迷いを正直に書く。
「この条件は変更される可能性がある」「向いてない人もいる」「編集部も実際の運用はしていない」を隠さない。
...
- 抽象論は書かない。一次情報で確認できた数字・条件・日付の具体例から始める
- 1文は短く。長い説明は「---」や改行で区切る
- 説教しない。「一緒に考えよう」「調べるまで分かってなかった」スタンス
...
- 区切りは「---」を使う
- 見出しは「【】」形式
- 最後は「興味があれば読んでみてください」調のソフトCTA（誘導先はブログ記事URLのみ。他チャネルへは誘導しない）
...
- 編集部の一人称体験・エピソードは書かない（実運用していないため虚偽になる）。代わりに「同じ状況の人が多い」という一般化と、調べてわかった事実で埋める
- 中盤以降に「こういう状況で調べ始めたのがFXだった」という流れで登場させる
- FXは「正解」として推さない。「調べた結果、選択肢のひとつとして残ったもの」として紹介する
- 最後にCTAへのソフトリンクを置く`
...
const shortformResponse = await client.messages.create({
  model: 'claude-sonnet-4-6',
  max_tokens: 2000,
  system: `あなたはtsumiba編集部として、Xの短文投稿（200字以内）を書くライターです。

【設計原則】
- 読者がヒーロー。編集部は「読者の代わりに条件を調べて整理する調査者」として登場する（体験者・成功者としては書かない）
- 「お金が不安な普通の人」の感情から入る。最初からFX・投資の話にしない
- Affinity（親近感）ファースト：共感・代弁から始めて、情報で終わる
- 透明性：「調べても分からなかった」「確認できていない」「向かない人もいる」を隠さない
- ソフトプッシュ：押し売り禁止。「気になったら」「興味があれば」調で終わる

【文体】カジュアル・一次情報で確認できた数字ファースト・絵文字なし・断定しない
【禁止表現】確実に儲かる・必ず増える・元本保証・誰でも稼げる・絶対儲かる

【短文5タイプのテンプレート（書き方の型）】
逆説型：「〜だと思ったけど実は逆だった」という意外な事実で興味を引く
検証記録型：「調べてわかったこと」を数字・出典・条件を入れて具体的に書く（一人称の売買体験は書かない）
