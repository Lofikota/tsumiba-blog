...
// ─────────────────────────────────────────
// 先生プロンプト（知識ベースから自動生成された出力ルール）を読み込む
const teacherPromptPath = path.join(ROOT, '../AI運用/yomi-teacher-prompt.md');
...
4. 本文4,500字以上（frontmatter・import行は含まない）
5. 末尾に【免責事項】と著者情報（田中蓮）

【絶対に書いてはいけないこと】
- 確実に儲かる・絶対に損しない・必ず増える・誰でも稼げる
- 「元本保証」という語句は肯定・否定問わず一切使わない。リスク説明は「元本割れリスクがある」「損失が生じる可能性がある」と書く
...
${teacherPrompt ? `【先生プロンプト（YMYL/E-E-A-T/PASBECONA/禁止表現の全ルール）】\n${teacherPrompt}` : ''}`;
...
const response = await client.messages.create({
  ...
  messages: [
    {
      role: 'user',
      content: `以下のキーワードで、田中蓮として4,500字以上のブログ記事をMDX形式で書いてください。
