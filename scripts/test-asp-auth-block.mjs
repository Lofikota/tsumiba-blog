...
const cases = [
  ...
  // ⚠️ ここは「特定の言い回し」ではなく「4点セットの構成要素が揃っているか」を見る。
  //   旧版は `Claude アイコンからサイドパネル.*Sign in` という散文にピン留めしていたため、
  //   2026-08-07 MON-C01-R で原因が変わり文言を直しただけでこのケースが落ちた。
  //   復旧文言は原因が変わるたびに書き換わる＝散文を握ると「正しい更新」が退行として報告される。
  //   握るべきは、押す画面（手順）・対象物・効果・完了確認が1つも欠けていないことの方。
  ['🚨には人間タスク（何をする/どうなる/手順/完了確認）が付く',
    F('crit-fix', state([app({ authorized_at: ymdAgo(6) })])),
    /🚨 .*【人間タスク】[\s\S]*chrome:\/\/extensions[\s\S]*1Password[\s\S]*3経路が同時に開く[\s\S]*完了確認＝/],
